...
YoloDetOpenVinoImpl::YoloDetOpenVinoImpl()
{
...
void YoloDetOpenVinoImpl::release()
{
	m_model.reset();
	m_model_status = false;
}

int YoloDetOpenVinoImpl::SetModel(std::string modelPath)
{
	try {
		release();
		if (modelPath.empty()) {
...
#ifdef _WIN32
		std::wstring wide_path = ToWideString(modelPath);
		if (wide_path.empty()) {
			std::cerr << "Error: Failed to convert model path to wide string." << std::endl;
			return -1;
		}
		m_model = m_core.read_model(wide_path);
#else
		m_model = m_core.read_model(modelPath);
#endif
		if (!m_model) {
			std::cerr << "Error: Failed to read model with OpenVINO." << std::endl;
			return -1;
		}

		// 编译模型到 CPU 设备（OpenVINO 的 CPU 插件支持 Intel CPU 优化）
		...
		std::cerr << "Error: OpenVINO exception: " << e.what() << std::endl;
		release();
		return -1;
	}
...
void YoloDetOpenVinoImpl::preprocess(cv::Mat& img, float data[])
{
	int w, h, x, y;
...
void YoloDetOpenVinoImpl::postprocess(float* output, size_t det_count, int img_width, int img_height, YoloDetResult& res)
{
	// 计算缩放比例和填充
...
int YoloDetOpenVinoImpl::Inference(cv::Mat image, YoloDetResult& res)
{
	if (!m_model_status) {
	...
	// 创建输入张量并设置数据
	ov::Shape input_shape = { 1, 3, YOLO26_INPUT_SIZE, YOLO26_INPUT_SIZE };
	ov::Tensor input_tensor(ov::element::f32, input_shape, m_h_input.data());
	...
	float* output_ptr = output_tensor.data<float>();
	const size_t output_elems = output_tensor.get_size();

	if (output_elems < YOLO26_OUTPUT_DIM) {
		std::cerr << "Error: Invalid OpenVINO output size." << std::endl;
		return -1;
	}
	...
	if (output_elems % YOLO26_OUTPUT_DIM != 0) {
		std::cerr << "Warning: OpenVINO output size is not a multiple of "
...
int YoloDetOpenVinoImpl::Inference(tag_camera_data4det image, YoloDetResult& res)
{
...
int YoloDetOpenVinoImpl::InferenceBatch(std::vector<cv::Mat>& images, std::vector<YoloDetResult>& results)
{
	if (!m_model_status) {
		std::cerr << "Error: Model not loaded, Set model please!" << std::endl;
		return -1;
...
cv::Mat YoloDetOpenVinoImpl::createMatFromData(tag_camera_data4det* camera_Data)
{
	return createMatFromData(camera_Data->width, camera_Data->height, camera_Data->channels, camera_Data->data);
}

cv::Mat YoloDetOpenVinoImpl::createMatFromData(int width, int height, int channels, unsigned char* data)
{
	// 确定图像类型
	int type;
	if (channels == 1) {
